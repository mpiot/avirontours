...
class PromoteUserCommand extends RoleCommand
{
    ...
    protected function configure(): void
    {
        parent::configure();
        $this
            ->setDescription('Add a role to specified user')
        ;
    }
    ...
    protected function executeRoleCommand(EntityManagerInterface $entityManager, SymfonyStyle $io, $email, $role): int
    {
        ...
        if (false === ($key = array_search($role, $user->getRoles(), true))) {
            ...
            $io->success(sprintf('Role "%s" has been added to user "%s". This change will not apply until the user logs out and back in again.', $role, $email));
        } else {
            $io->error(sprintf('User "%s" did already have "%s" role.', $email, $role));
        }
